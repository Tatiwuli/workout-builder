...
def render_muscle_selection():
    ...
    all_selected = [muscle for muscle in PULL_OPTIONS + PUSH_OPTIONS + LEG_OPTIONS if st.session_state.get(muscle)]
    st.write(", ".join(all_selected) if all_selected else "No muscles selected.")
...
def render_questionnaire():
    ...
    with col1:
        st.write(", ".join(st.session_state["all_selected_muscles"]) or "No muscles selected.")
...
if __name__ == "__main__":
    ...
    if st.session_state["workflow_stage"] == "muscle_selection":
        ...
        if not api_key:
            st.error("Please insert your OpenAI API key in the home page to generate your plan")
